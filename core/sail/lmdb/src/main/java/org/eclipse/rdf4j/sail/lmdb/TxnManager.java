...
//	private final ReadWriteLockManager lockManager = new ReadPrefReadWriteLockManager();
	 ...
	 * {@link ReadWriteLockManager#getWriteLock()}. This is required to block transactions while
	 * automatic resizing the memory map.
	 *
	...
	class Txn implements Closeable, AutoCloseable {
		...
		/**
		 * A {@link ReadWriteLockManager#getReadLock()} should be acquired while working with the transaction.
		 * This is required to block transactions while automatic resizing the memory map.
		 *
		 * @return lock for managed transactions
		 */
		ReadWriteLockManager lockManager() {
